...
class Spoon(pygame.sprite.Sprite):
    def __init__(self, location, screenSize, speed):
      ...
      self.originalImage = pygame.transform.scale_by(self.originalImage, ((screenSize[0]*0.415)/width))
    ...
    def update(self, playerPosition):
      ...
      if self.difficultyLevel == "EASY":
        if self.state == "IDLE":
          self.rect.y = -(self.maxY*0.3)
          if self.stageTimer %90 == 0:
            self.state = "STALK"
        elif self.state == "STALK":
          self.rect.y = self.maxY *0.04
        ...
        if self.state == "IDLE":
          ...
          if self.rect.bottom <= 0:
            self.state = "IDLE"
      elif self.difficultyLevel == "HARD":
        if self.state == "IDLE":
          self.rect.y = -(self.maxY*0.3)
          if self.stageTimer %60 == 0:
            self.state = "STALK"
        elif self.state == "STALK":
          self.rect.y = self.maxY *0.04
          if self.rect.centerx < playerPosition[0]:
